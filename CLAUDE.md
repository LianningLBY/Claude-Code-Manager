...
- **Frontend Test Harness**: `TestHarnessService` 是普通对话、一次性按钮、Goal、固定 URL 和当前工作区的统一门面；`TestHarnessRun/Attempt/Event/Evidence/Finding` 持久化冻结的 Target/TestPlan/runtime、精确 SHA、阶段事件、内容哈希证据、结构化结论和稳定 finding fingerprint。公开 API 为 `/api/tasks/{id}/test-runs`（start/list/get/cancel/repeat/compare/evidence），Agent MCP 走同一 internal start/status/stop；旧 Browser/Workspace API 只保留兼容 adapter，不得另建平行状态源。每个 Task 同时只允许一个 active run，idempotency key 必须绑定完整输入指纹；重启后先把受管 job 目录归档为持久证据，再将未终态 run fail closed；Goal 只接受最新、非 stale、已清理且包含截图/报告的 passed run。PR/ref 在真正的隔离 sandbox 接入前必须从 API、MCP、意图路由和底层 Target manager 一致 fail closed，禁止在 Manager 宿主机 fetch、build 或执行不可信提交，也禁止改测当前工作区。

...
- **清理边界**：截至本记录，PR #1170 仍为 OPEN 且未合并，DLV-1～3、Task 1009/1010/1012、对应 worktree/local branch 和测试远端分支仍待明确授权后删除；约 12.8GB 的 `claude_manager.db.pre_e2e_cleanup_20260816` 备份及服务器既有 PEM/CSV 等无关文件必须保留。清理完成后需再次执行 GitHub 状态、远端分支、数据库关联计数、`foreign_key_check`、worktree/local branch、服务与 HTTP 审计。

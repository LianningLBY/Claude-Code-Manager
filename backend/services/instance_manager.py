        ...
        *,
        ...
        async with self.db_factory() as db:
            ...
            if browser_binding_id is not None:
                ...
                observed_binding = await db.get(
                    TestHarnessChildBinding,
                    browser_binding_id,
                )
                if observed_binding is None:
                    raise LaunchSupersededError(
                        "Browser Agent durable launch binding disappeared"
                    )
                owner_fence = await db.execute(
                    update(Task)
                    .where(
                        Task.id == observed_binding.owner_task_id,
                        Task.incarnation_id
                        == observed_binding.owner_task_incarnation_id,
                    )
                    .values(
                        incarnation_id=observed_binding.owner_task_incarnation_id
                    )
                )
                if owner_fence.rowcount != 1:
                    raise LaunchSupersededError(
                        "Browser Agent owner generation disappeared at provider boundary"
                    )
                if observed_binding.harness_run_id:
                    await db.execute(
                        select(TestHarnessRun)
                        .where(TestHarnessRun.id == observed_binding.harness_run_id)
                        .with_for_update()
                    )
                if observed_binding.workspace_review_run_id:
                    await db.execute(
                        select(WorkspaceReviewRun)
                        .where(
                            WorkspaceReviewRun.id
                            == observed_binding.workspace_review_run_id
                        )
                        .with_for_update()
                    )
            ...
            if browser_binding_id is not None:
                ...
                boundary_error = await browser_child_identity_error(
                    db,
                    browser_binding,
                    task,
                    expected_states={CHILD_RUNNING},
                    expected_instance_id=instance_id,
                    expected_retry_count=task_retry_count,
                )
                if boundary_error is not None:
                    raise LaunchSupersededError(
                        "Browser Agent launch identity changed at provider "
        ...
        async with self.db_factory() as db:
            ...
            if task_id is not None:
                ...
                supplied_browser_job_id = (
                    enabled_skills.get(BROWSER_REVIEW_SKILL)
                ...
                projected_browser_child = bool(
                    ...
                    or (
                        isinstance(task.enabled_skills, dict)
                        and BROWSER_REVIEW_SKILL in task.enabled_skills
                    )
                    or supplied_browser_job_id is not None
                )
                if browser_binding is None and projected_browser_child:
                    raise LaunchSupersededError(
                        "Browser Agent launch has no durable child binding"
                    )
                if browser_binding is not None:
                    ...
                    if identity_error is not None:
                        raise LaunchSupersededError(
                            "Browser Agent durable launch identity rejected: "
                            f"{identity_error}"
                        )
                    if (
                        ...
                        or supplied_browser_job_id
                        != browser_binding.browser_review_job_id
                        or enable_workflows
                        or resume_session_id is not None
                    ):
                        raise LaunchSupersededError(
                            "Browser Agent launch no longer matches its frozen "
                            "provider/model/effort/job/skill tuple"
                        )
                    browser_review_task = True
                    browser_binding_id = browser_binding.id

...
- **默认 Provider**: 新任务默认使用 `codex`，Codex 默认模型为 `gpt-5.6-sol`；均可通过 `DEFAULT_PROVIDER` / `DEFAULT_CODEX_MODEL` 覆盖。所有 Task 创建入口（含 standalone Plan 物化执行 Task）必须显式持久化统一解析后的 provider/model/effort，禁止依赖 ORM 中为兼容旧库保留的 Claude 默认值

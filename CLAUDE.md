...
- **Skills 系统**: Task.enabled_skills（JSON dict，如 `{"monitor": true}`）控制启用哪些 Skill/MCP 工具。启用只表示能力可发现，不等于用户已经调用；首次任务和普通续聊严禁追加“用户通过 `$command` 触发”的模板，只有消息开头真实解析到 `$command` 时才注入显式调用指令。创建 task 时勾选 Skills，dispatcher 根据 enabled_skills 动态生成 provider 对应的 MCP config。聊天页的任务级 Distill 必须按 `Task.provider` 分流；Codex 使用绑定/健康账号运行独立 `codex exec --ephemeral`，不得复用或改写原 task 的 thread/账号绑定。该临时进程必须通过 `InstanceManager.codex_home_exec_guard` 纳入同一 CODEX_HOME maintenance 准入，取消/超时须 kill + reap 后才释放 home；对话属于不可信输入，Codex distill 必须使用只读 sandbox 并忽略用户 config/rules，严禁 `--dangerously-bypass-approvals-and-sandbox`
...
- **MCP 架构**: `mcp_config.py` 先为主任务、Monitor、Sub-Agent 构造 provider-neutral `McpServerSpec`（command/args/cwd/env/required/enabled_tools/timeouts），再由 provider renderer 输出配置：Claude renderer 生成兼容的 `mcpServers` JSON；Codex renderer 生成 app-server 线程级 `config.mcp_servers` 对象，或从同一对象生成 `codex exec -c` TOML argv（纯内存、不写全局 `config.toml`）。`CODEX_MAIN_MCP_ENABLED=false` 默认关闭主任务试运行；开启后 `InstanceManager` 每次 start/resume/重试/换号都重建 task-scoped `ccm_skills` required spec，`CodexAppServer.start_turn` 只合并进本次 thread 的 config；从 transport 启动、thread/start|resume 到 turn/start 的任一异常或缺失 thread/turn id 响应都转为 `CodexRequiredMcpError`，最终 adapter 兜底同样 fail closed，绝不得降级到无工具 exec。exec fallback 接线、Codex skill prompt 和 UI 仍未开放。三个 FastMCP server 分别是 `ccm_skills_server.py`、`ccm_monitor_agent_server.py`、`ccm_sub_agent_server.py`，通过 stdio 与 CLI 通信、通过 HTTP 调用 CCM API。Claude 配置文件动态生成到临时目录并在对应进程结束后清理

...
- **Worker Plan 重启恢复与精确取消（2026-08-08）**: Manager 每次 Worker-owned Run claim 都原子写 `PlanAgentWorkerDispatchReceipt`。`prepared` 才能证明未调用远端并安全重排；进入 `remote_possible` 前必须冻结 payload digest，之后 shutdown/transport/permanent 异常一律保留原 generation，由 Worker 的 exact-import audit 对账，禁止盲目重发 `worker-import`。用户取消必须先把 Manager Run 以 exact generation 持久化为 `cancelling`，再按 `run_id + plan_id + payload_digest + manager_v1` 发 RPC；Worker 用永久 `PlanAgentWorkerImportReceipt` 在 import 与 cancel 间原子仲裁，cancel 先到时留下 `cancelled_before_import` tombstone，已导入/已删除的历史 identity 也不得复活。Worker Run 的 two-valued SQL gate 还要求 `manager_v1 → import_receipt_protocol=1`、其他 Run → `NULL`；当前 importer 必须先 flush receipt 再写 protocol，旧二进制因省略 marker 由数据库拒绝。ACK 丢失或 Manager 重启后持续重放同一 exact cancel；durable fence 的 commit、旧 dispatch lifecycle 的锁外 reap 都必须在 caller cancellation 下 shield-settle，reap 暂时失败时 recovery 每轮先重试 reap，不能让 live lifecycle 永久挡住 exact RPC。若 Worker 的 completed/failed 终态先获胜，必须认证并导入真实 Run/Version 子图，不能伪造 cancelled。旧 Worker 未声明 reconciliation/exact-cancel protocol 必须 fail closed；dispatch receipt 只有 `settled` 才可随 Plan 图删除，import receipt 永久保留，存在 tombstone 或已脱离 exact 图的历史 receipt 时 migration downgrade 必须拒绝。MySQL 的非事务 downgrade 只接受零 import/receipt history，并按 receipt-empty gate → no-manager Run gate 切换 writer protocol；任一历史行都拒绝降级。
- **Worker destroy 的 Plan 证据门禁**: 销毁必须直接按冻结的 `PlanAgentWorkerDispatchReceipt.worker_id` 审计 dispatch ownership，不能只从当前 Run/Plan 的 `worker_id` 反查；即使 Run 已改绑、脱离或丢失，未 settled、malformed 或 identity 漂移的 receipt 仍阻止销毁。terminal Run 也必须经 `runtime_run_is_clean()` 验证完整 Step/generation/连续 attempt/receipt shape，不能只信 `status='cleaned'`。
- **Task 删除与 Plan runtime**: Task 删除先在同一 Task writer fence 下按稳定 Run→Plan 顺序锁定完整 first-class Plan 图；terminal target-owned Plan 可与 Task 原子级联，外部 Plan 指向 execution Task 的 Application 审计必须保留。旧 `Task(mode=plan)` 的纯 legacy Run 只有在 terminal、无 owner/外部引用且 `runtime_run_is_clean()` 验证每个 Step/generation/连续 attempt receipt 后才可连同 cleaned receipt 删除；缺失/非 clean/malformed runtime 直接拒绝，Worker 权威 DELETE 的回调也必须晚于这层完整 preflight。
- **Standalone execution Task 物化栅栏**: `plan_service.materialize_execution_task` 完成 staleness 检查后先结束旧读事务，按统一的 Worker → Plan 顺序先取得 Worker lifecycle writer fence，再以 `Plan.lock_version` 条件写锁定聚合（避免 SQLite WAL `SQLITE_BUSY_SNAPSHOT` 及与 create-Run 的反向死锁），随后刷新并复核 exact Plan/Version、在同一事务重验当前 ACL，原子提交 inline approval、Task 与 `PlanApplication`。ACL 依赖刷新只允许 `populate_existing` 读；不得在已持 Plan 锁后追加 Project/第二 Worker 写锁形成跨 Worker 环。staleness 检查期间若 archive、Refresh/new Run 或 current Version 变化必须稳定返回 409；Task staging/commit 失败须连同 approval 和 lock_version 回滚，不能留下已批准但未物化的 Version。
- **Legacy Plan Worker carrier 恢复（2026-08-06）**: 升级前已在 Worker 物化的 approved `Task(mode=plan)` 绝不能通过通用 Task POST 重建。Manager 先订阅并隔离事件，再用双侧 approved Version/Application proof、portable execution fingerprint、完整 snapshot/history 和前后两次 proof 稳定读回精确采纳；缺失或语义不一致直接 `conflict` 并退订，禁止盲重放。首次采纳后在同一 Task operation lock 内释放事件隔离并再次闭合读回，防止 terminal event 落在 proof→退出窗口。
- **Capability Core（2026-08-05）**: Plan 与 Pre-PR Code Review 是可复用、provider-neutral 的 Capability，不是 Delivery Controller 私有实现。`CapabilityInvocation` 冻结 Task generation、request、executor config 与 policy，`CapabilityExecution` 保存 attempt/handle/output；状态推进统一使用 Task-scoped lock + state-version CAS，等待 Plan 输入时不占 Developer Instance。`source=delivery_controller` 是 required gate；普通 Auto Task 可通过 human-facing Capability API 创建 `source=human_request` 的 advisory invocation，模型自助请求使用 `source=agent_request` 且只能由 exact terminal-output admission 创建，三者都必须复用同一 runner。Capability-owned Plan 只允许读取和回答 `waiting_user` 输入，其他 Plan API 写操作与 Reviewer Task 的 chat/inject/retry/edit/share 都必须拒绝；`PlanResource.ownership/read_only` 必须扫描任意历史 Run 的 Capability marker 后 fail-closed 投影，UI 仅保留输入、导航和审计，完成结果不得进入普通 Review/Execute 行动卡。`CAPABILITY_CORE_ENABLED` 只限制新 admission；数据库中已接纳的 queued/running/cancelling work即使重启后开关关闭也必须继续恢复或收口。
- **Auto Capability policy 与恢复（2026-08-07）**: `Task.capability_policy` 只允许本机普通 `mode=auto` Task 在创建时显式设置，SQL `NULL` 是唯一关闭态；V1 严格结构为 `{version:1,max_invocations,capabilities:{plan?,code_review?}}`，映射键同时是静态白名单和分类预算，总/分类都为正整数且硬上限 8。policy 按 Task incarnation 冻结，PUT、Manager→Worker create、migration、Shared shadow、Delivery/Plan/Loop/Goal 都 fail closed，clone 不继承。预算以 `source=agent_request` 的逻辑 Invocation 作不可退款 ledger（失败/取消/stale 也计数，Execution retry 和幂等重放不重复计）。`agent_capability_admission.py` 只从 exact retry/turn/source/Instance 的最终 terminal output 严格解析请求，并在同一 Task 锁事务内创建 Invocation/首个 Execution/resume outbox、消费预算和提交 `waiting_capability`；`capability_result.py` 反向验证唯一 completed Execution 与不可变结果图；`capability_resume.py` 物化冻结 payload，由 coordinator 经 lease outbox 恢复同一 Task/session 的 G→G+1，并在 provider boundary 前后持久化 launch/settlement 证据。Manager 重启或 ACK 丢失只重放可证明未越过 provider boundary 的 exact outbox；`launched/completed` 或证据不完整一律 fail closed。Dispatcher 只在 resume Phase 1（claim、launch、provider-boundary promotion 与 prelaunch cleanup）持 `capability_task_lock`，进入等待 output consumer 的 Phase 2 前必须释放，否则 terminal settlement 会自锁；同一协程不得递归获取该非重入锁。stop/cancel 必须先关闭 resume admission、静止已 dequeue consumer并终结 Capability/Execution/outbox，最后才能终态化 Task，合法 pre-provider claimed G+1 只能恢复/取消同一代，禁止生成 G+2。公开 Capability consume/cancel 只允许 `human_request + attach_only`，Agent/Controller-owned invocation 必须走所属 Task/Controller 生命周期。`AUTO_CAPABILITY_ENABLED=false` 是独立的新 admission 开关且还要求 Capability Core 开启；它只关闭新模型请求，已接纳 outbox 仍须恢复或收口。
- **Logical turn / terminal arbitration（2026-08-06）**: `Task.turn_generation` 是跨 provider 的逻辑 turn 代次，只有新 admission 才递增；retry/账号切换保持不变。所有执行日志和实时事件都携带 `task_retry_count + task_turn_generation`，可用时再带 `native_turn_id`，Manager/前端只能合并 exact generation。`Task.turn_source_log_id` 是当前 exact turn 的权威 source 指针，可指向可见用户日志或隐藏 `turn_source` alias；`LogEntry.turn_scope` 的非空值只允许 `source/foreground/autonomous/orphan`，终态解析只考虑同 retry/generation/source 且与 source 的真实 `instance_id` 一致的 `foreground`，缺少本地 Instance provenance 的 Worker 镜像不能充当 Auto 终态授权。Claude/Codex 都以最后一条 in-scope terminal 为准，后到的 failed/interrupted/malformed 必须否决早先成功；Codex app-server 必须显式持久化 completed+success 或失败 status+error。Context overflow 只接受 exact Codex structured preflight；proof→summary→writer 和 `compact_retry` 队列全程携带不可变 Task generation permit，并在每个 await/launch 边界复核，cancel/retry 后不得复活。未来 parser 必须从 Task pointer 加载 source，并以实际 transport 持久化的 native/terminal proof 做裁决，禁止仅凭相邻日志或旧成功猜测。
- **Loop / Goal exact terminal proof（2026-08-07）**: 每个 mode turn 必须无超时等待 exact output consumer 完整结算，且只有 provider-semantic `exit_code == 0` 才能消费 Loop signal、运行 Goal evaluator、授权续轮或提交成功。proof 绑定 Task retry/turn、source/Instance、exact process、prelaunch log boundary、`loop_iteration`、terminal log/native turn；Goal transcript 只取同代同 Instance 的 foreground、非错误 source→terminal prefix。signal repair 先消费前轮 proof，再由 repair turn 自身的 exit-0 proof 重新授权；evaluator、广播等 await 后及最终 complete/continuation 前必须重验 terminal tail，晚到 fatal/malformed terminal 一律否决旧成功。
- **Worker / Plan admission recovery（2026-08-06）**: Manager→Worker chat 在一个事务中写 user Log、G→G+1 Task marker 和 Manager receipt，Worker 在一个事务中写 user Log、accepted receipt 与可选 Plan outbox，真正 dequeue 时再把 Task/Log/receipt 原子推进为 G+1/launched；Plan prelaunch 终结必须在同一事务取消 linked handoff。普通 Plan 与 Worker queued admission 在 commit 成功但 ACK 丢失（含 cancellation）时，必须丢弃未知 session，再用 fresh session 按 Task→Worker→Plan 锁序核对完整 source、payload、route 和 launch evidence；只有确认 provider boundary 尚未调用，才恢复原 status/owner、把 Plan `launching` 退回 `queued` 并保留同一 G/source，随后再传播 boundary 前的 graceful cancellation。任何已越过 boundary 或证据不完整的路径都保持 uncertain、禁止盲重放。Manager 在首次 POST 前启动持续恢复，Worker 启动时主动重放 accepted receipt；marker 未收敛前禁止 retry/migration/执行配置修改和 Worker 销毁 fallback。Worker `cancel`/`stop-session` 的远端 mutation、终态 GET、Manager mirror CAS 与发布必须完整持有同一 Task operation lock；transport/5xx/非法 ACK 一律落 exact-generation durable quarantine，阻断 retry/chat/handoff/G+1 recovery/migration/destroy，仅同 Worker retry/turn 的权威终态 readback 可清除。Worker destroy 在 `ready|stopped|error → destroying` CAS 后冻结 opaque exact lifecycle claim；活动 Task 的 stop 仍走同一 POST→终态 GET→quarantine/CAS 协议，公开 proxy 不放宽 `destroying`，旧 coordinator 也不能复位新 lifecycle。Worker source 迁移只接受 `plan_review/completed/failed/cancelled/conflict` 等明确 inert 状态，本地 `merging` 也不可迁移；destroy 的 lossy detach 必须复用同一 inert + handoff/quarantine gate。
- **Worker termination durable receipt（2026-08-07）**: Manager 的 cancel/stop-session/destroy-stop/supersede 先持久化 exact receipt，再按远端 GET → 仅缺失时 PUT → Manager result CAS → ACK 对账；旧 `POST /terminate-generation` 恒定 409 且无副作用，Worker-managed 本地副本也拒绝无 receipt 的公共 cancel/stop-session。active receipt 与所有 Task 终态/lifecycle writer 共用 Task-side CAS，覆盖 completion/failure、source-aware finalizer、mode claim、queued recovery、stale reset 与首次 provider effect；queue/Plan/signal/terminal/publication 的每个副作用都按 Task→receipt→Instance 锁序，以锁后 fresh UTC 重新验证 lease，广播跨期后不得继续发布。先提交者独占 exact generation；无权威 canonical terminal 的 dead lifecycle 只能 fail closed 为 `failed` 并释放 Instance，绝不能伪造 `completed`。
- **Terminal arbitration migration fence（2026-08-06）**: PostgreSQL 在 preflight 前取得 `ACCESS EXCLUSIVE`；SQLite 先写 `alembic_version` 取得 writer transaction，避免检查后并发写穿。MySQL 仅支持 8.0.16+ 且三张相关表必须为 InnoDB，MariaDB fail closed；phase ALTER 必须单语句原子安装列/唯一约束，并用 shadow/gate/canonical CHECK 保证约束交换和 Alembic stamp 窗口始终有 guard。重跑须按精确 schema 恢复 partial/complete DDL，并验证列类型/nullable、唯一列顺序、CHECK 表达式与 `ENFORCED=YES`，不能无条件重复 zero-row preflight。
...
>
> **Plan runtime receipt rollout 门禁**：`8d2f5b7a1c90` 按设计 fail closed。升级前必须人工收敛旧 `running`/其他非 terminal 或 malformed Plan Step，以及旧 `cancelling` Run；migration 不会伪造“未启动”证明。降级越过该 revision 前，所有 runtime receipt 必须是完整、连续且与 Step/Run identity 一致的 `cleaned` 证据，并且不能有 active cancellation。命中门禁时保持停服并先对账/清理，禁止绕过或手改 schema。

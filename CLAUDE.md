...
- **Logical turn / Worker handoff fence（2026-08-06）**: `Task.turn_generation` 是跨 provider 的逻辑 turn 代次，只有新 admission 才递增；retry/账号切换保持不变。所有执行日志和实时事件都携带 `task_retry_count + task_turn_generation`，可用时再带 `native_turn_id`，Manager/前端只能合并 exact generation。Manager→Worker chat 在一个事务中写 user Log、G→G+1 Task marker 和 Manager receipt，Worker 在一个事务中写 user Log、accepted receipt 与可选 Plan outbox，真正 dequeue 时再把 Task/Log/receipt 原子推进为 G+1/launched；Plan prelaunch 终结必须在同一事务取消 linked handoff。Manager 在首次 POST 前启动持续恢复，Worker 启动时主动重放 accepted receipt；marker 未收敛前禁止 retry/migration/执行配置修改和 Worker 销毁 fallback，任何无法证明 launched/cancelled/completed 的路径都 fail closed。

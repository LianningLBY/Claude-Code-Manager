...
- **Markdown 数学公式**: 所有聊天与 Discussion 的 Markdown 必须统一经 `components/Markdown/MarkdownRenderer.tsx` 渲染。公式支持 `$...$` / `$$...$$`，并在解析前把 Codex 常见的 `\\(...\\)` / `\\[...\\]` 完整配对转换为等价分隔符；转换必须跳过 fenced code 和 inline code，未闭合分隔符保持原文，禁止对消息做无上下文的全局替换

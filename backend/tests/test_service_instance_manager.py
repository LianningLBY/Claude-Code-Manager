...
from backend.models.test_harness import (
    TestHarnessChildBinding as BrowserChildBinding,
...
async def _add_running_browser_child(db, task: Task, *, job_id: str) -> None:
    """Persist the durable launch authority required by Browser child tests."""

    owner = Task(
        title="browser review owner",
        status="completed",
        provider=task.provider,
    )
    db.add(owner)
    await db.flush()
    run_id = f"{owner.id:032x}"
    task.enabled_skills = {"browser-review": job_id}
    task.metadata_ = {
    ...
    run = HarnessRun(
        ...
        runtime_config={"provider": task.provider},
    ...
    db.add(
        BrowserChildBinding(
            ...
            state="running",
            claimed_retry_count=task.retry_count,
            claimed_instance_id=task.instance_id,
        )
    )
...
async def test_codex_browser_review_requires_mcp_only_app_server(
    ...
    async with db_factory() as db:
        ...
        await _add_running_browser_child(db, task, job_id="job-abc")
        await db.commit()
        await db.refresh(inst)
        await db.refresh(task)
...
async def test_codex_browser_review_uses_proven_mcp_only_profile(
    ...
    async with db_factory() as db:
        ...
        await _add_running_browser_child(db, task, job_id="job-bound")
        await db.commit()
        await db.refresh(inst)
        await db.refresh(task)
...
@pytest.mark.asyncio
async def test_browser_binding_drift_is_rechecked_at_provider_boundary(
    db_factory, monkeypatch, tmp_path,
):
    monkeypatch.setattr(settings, "codex_app_server_enabled", True)
    monkeypatch.setattr(settings, "codex_main_mcp_enabled", False)
    async with db_factory() as db:
        inst = Instance(name="browser-boundary-drift")
        db.add(inst)
        await db.flush()
        task = Task(
            title="browser boundary drift",
            status="executing",
            provider="codex",
            instance_id=inst.id,
        )
        await _add_running_browser_child(db, task, job_id="job-boundary")
        await db.commit()
        await db.refresh(inst)
        await db.refresh(task)

    async def drift_before_provider_effect(**kwargs):
        async with db_factory() as db:
            binding = await db.scalar(
                select(BrowserChildBinding).where(
                    BrowserChildBinding.child_task_id == task.id
                )
            )
            binding.model = "drifted-after-launch-validation"
            await db.commit()
        await kwargs["on_launch_admitted"]()
        return 1234
    ...
    with patch.object(
        im,
        "_launch_codex_app_server",
        new=AsyncMock(side_effect=drift_before_provider_effect),
    ):
        with pytest.raises(
            LaunchSupersededError,
            match="provider boundary",
        ):
            await im.launch(
                instance_id=inst.id,
                prompt="never cross with a drifted identity",
                task_id=task.id,
                cwd=str(tmp_path),
                provider="codex",
                enabled_skills=task.enabled_skills,
                config_dir=str(tmp_path / "browser-boundary-home"),
            )
...
async def test_claude_browser_review_disables_builtins_but_keeps_bound_mcp(
    ...
    async with db_factory() as db:
        ...
        await _add_running_browser_child(db, task, job_id="job-claude")
        await db.commit()
        await db.refresh(inst)
        await db.refresh(task)

...
- **Skills 系统**: Task.enabled_skills（JSON dict，如 `{"monitor": true}`）控制注入哪些 MCP 工具。启用只表示能力可发现，不等于用户已经调用；只有消息开头真实解析到 `$command` 时才注入显式调用指令。新建 Task 的 description 以已注册 `$command` 开头时，dispatcher 必须像 chat 续轮一样去掉命令前缀，并在 execution ownership commit 中临时写入 required skills，使首个 MCP 调用能通过 API gate；initial/queued 两路都必须先取得 Task 写屏障并刷新最新 skills/metadata，再以 generation token 叠加临时值，turn 结束只清理仍属于该 token 的 command keys，显式用户保存会清 marker，绝不能用 dequeue 旧快照覆盖并发修改。`selected_user_skills` 通过应用 async session 读取；默认 Claude PTY 没有 append-system-prompt-file 接口，须把小型 L0 skill 目录前置到本 turn prompt，非 PTY 仍传临时文件。创建 task 时勾选 Skills，dispatcher 根据 enabled_skills 动态生成 provider 对应的 MCP config。聊天页的任务级 Distill 必须按 `Task.provider` 分流；Codex 使用绑定/健康账号运行独立 `codex exec --ephemeral`，不得复用或改写原 task 的 thread/账号绑定。该临时进程必须通过 `InstanceManager.codex_home_exec_guard` 纳入同一 CODEX_HOME maintenance 准入，取消/超时须 kill + reap 后才释放 home；对话属于不可信输入，Codex distill 必须使用只读 sandbox 并忽略用户 config/rules，严禁 `--dangerously-bypass-approvals-and-sandbox`

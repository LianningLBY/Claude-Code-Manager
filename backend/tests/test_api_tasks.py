...
@pytest.mark.asyncio
async def test_public_update_cannot_mutate_durable_browser_child(
    client,
    session_factory,
):
    from backend.models.task import Task
    ...
    async with session_factory() as db:
        owner = Task(title="Browser owner", status="completed")
        child = Task(
    ...
    response = await client.put(
        ...
        json={"model": "attacker-selected-model"},
    )

    assert response.status_code == 409
    assert "frozen Harness binding" in response.json()["detail"]
    async with session_factory() as db:
        child = await db.get(Task, child_id)
        assert child.model != "attacker-selected-model"


@pytest.mark.asyncio
async def test_delete_owner_removes_complete_terminal_harness_graph(
    client,
    session_factory,
):
    from backend.models.task import Task
    ...
    created = await client.post(
        "/api/tasks",
        json={"title": "Disposable Harness owner", "description": "done"},
    )
    owner_id = created.json()["id"]
    cancelled = await client.post(f"/api/tasks/{owner_id}/cancel")
    ...
    async with session_factory() as db:
        ...
        child = Task(
            title="Terminal Browser child",
            status="cancelled",
            provider="codex",
            model="gpt-5.6-sol",
            codex_service_tier="default",
            effort_level="high",
            enabled_skills={"browser-review": "terminal-job"},
            archived=True,
            completed_at=datetime.utcnow(),
            metadata_={
                "browser_review_job_id": "terminal-job",
                "test_harness_run_id": run_id,
                "workspace_review_run_id": None,
                "test_harness_parent_task_id": owner_id,
                "workspace_review_parent_task_id": owner_id,
                "isolated_browser_agent": True,
            },
        )
        db.add(child)
    ...
    response = await client.delete(f"/api/tasks/{owner_id}")

    assert response.status_code == 200, response.text
    async with session_factory() as db:
        assert await db.get(Task, owner_id) is None
        assert await db.get(Task, child_id) is None
        assert await db.get(TestHarnessRun, run_id) is None
        assert await db.get(TestHarnessAttempt, attempt_id) is None
        assert await db.get(TestHarnessChildBinding, binding_id) is None

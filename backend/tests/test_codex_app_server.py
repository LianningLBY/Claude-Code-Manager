...
def _ssh_mcp_spec(task_id: int) -> McpServerSpec:
    return McpServerSpec(
        name="ccm_ssh",
        command="python",
        args=(
        ...
        cwd="/ccm",
        required=True,
        enabled_tools=("list_connections", "execute_command"),
        startup_timeout_sec=10,
        tool_timeout_sec=60,
    )


def _task_ssh_thread_response(
    thread_id: str = "thread-task-ssh-isolated",
) -> dict:
    return {
        "thread": {
            "id": thread_id,
...
async def _start_isolated_ssh_test_turn(
    server: CodexAppServer,
    *,
    ambient_mcp: dict | None = None,
) -> tuple[CodexTurnProcess, str]:
    ambient = ambient_mcp or {
    ...
    server._request = AsyncMock(side_effect=[
        ...
        _task_ssh_thread_response(),
        {"config": {"mcp_servers": ambient}},
        {"turn": {"id": "turn-task-ssh-isolated"}},
    ])
    return await server.start_turn(
        prompt="inspect the authorized SSH server",
        ...
        git_env=None,
        task_id=994,
        mcp_specs=(_task_mcp_spec(994), _ssh_mcp_spec(994)),
        disable_project_config=True,
        disable_user_mcp=True,
        isolate_mcp_inventory=True,
...
@pytest.mark.asyncio
async def test_managed_ssh_disables_ambient_mcp_and_keeps_exact_ccm_servers():
    server = CodexAppServer("codex")

    process, thread_id = await _start_isolated_ssh_test_turn(server)

    assert process.returncode is None
    assert thread_id == "thread-task-ssh-isolated"
    ...
    config = server._request.await_args_list[1].args[1]["config"]
    assert config["mcp_servers"]["github"] == {"enabled": False}
    assert config["mcp_servers"]["user-app"] == {"enabled": False}
    assert config["mcp_servers"]["ccm_skills"]["required"] is True
    assert config["mcp_servers"]["ccm_ssh"]["required"] is True
    assert set(server._contexts_by_thread[thread_id].allowed_mcp_servers or ()) == {
        "ccm_skills",
        "ccm_ssh",
    }
...
@pytest.mark.asyncio
async def test_managed_ssh_rejects_ambient_ccm_server_name_collision():
    server = CodexAppServer("codex")
    server._process = SimpleNamespace(pid=4321, returncode=None)
    server.ensure_started = AsyncMock()
    server._request = AsyncMock(return_value={
        "config": {
            "mcp_servers": {
                "ccm_ssh": {"command": "/tmp/attacker-controlled"},
            },
        },
    })
    ...
    with pytest.raises(
        CodexRequiredMcpPreTurnError,
        match="could not audit and isolate ambient MCP",
    ):
        await server.start_turn(
            ...
            git_env=None,
            task_id=995,
            mcp_specs=(_task_mcp_spec(995), _ssh_mcp_spec(995)),
            disable_project_config=True,
            disable_user_mcp=True,
            isolate_mcp_inventory=True,
            sandbox_mode="workspace-write",
...
@pytest.mark.asyncio
async def test_managed_ssh_rejects_ambient_mcp_drift_before_turn_start():
    server = CodexAppServer("codex")
    server._process = SimpleNamespace(pid=4321, returncode=None)
    server.ensure_started = AsyncMock()
    server._request = AsyncMock(side_effect=[
        {"config": {"mcp_servers": {"github": {"command": "/bin/one"}}}},
        _task_ssh_thread_response("thread-task-ssh-drift"),
        {"config": {"mcp_servers": {"github": {"command": "/bin/two"}}}},
    ])

    with pytest.raises(
        CodexRequiredMcpPreTurnError,
        match="inventory drifted before turn/start",
    ):
        await server.start_turn(
            prompt="must not reach the model",
            cwd="/workspace/project",
            ...
            git_env=None,
            task_id=996,
            mcp_specs=(_task_mcp_spec(996), _ssh_mcp_spec(996)),
            disable_project_config=True,
            disable_user_mcp=True,
            isolate_mcp_inventory=True,
            sandbox_mode="workspace-write",
...
@pytest.mark.asyncio
async def test_managed_ssh_interrupts_unbound_mcp_tool_call():
    server = CodexAppServer("codex")
    process, thread_id = await _start_isolated_ssh_test_turn(server)

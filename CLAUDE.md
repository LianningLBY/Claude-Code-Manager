...
- **MCP 架构**: `mcp_config.py` 先为主任务、Monitor、Sub-Agent 构造 provider-neutral `McpServerSpec`（command/args/cwd/env/required/enabled_tools/timeouts），再由 provider renderer 输出配置：Claude renderer 生成兼容的 `mcpServers` JSON；Codex renderer 生成 app-server 线程级 `config.mcp_servers` 对象，或从同一对象生成 `codex exec -c` TOML argv（纯内存、不写全局 `config.toml`）。`CODEX_MAIN_MCP_ENABLED=false` 默认关闭主任务试运行；开启后 `InstanceManager` 每次 start/resume/重试/换号都重建 task-scoped `ccm_skills` required spec，并把同一份 spec 交给 app-server 或 exec。invalid config、turn/start 及之后的失败、未知 required-MCP 异常一律 fail closed；只有 transport 启动或 `thread/start|resume` 在 turn 提交前失败才标为 `CodexRequiredMcpPreTurnError`，且仅完整主任务 MCP 可回退到 MCP-equivalent exec。Sub-Agent 因需要 live thread control 始终只走 app-server，即使同一任务也开启完整主 MCP。Codex skill prompt 和 UI 仍未开放。三个 FastMCP server 分别是 `ccm_skills_server.py`、`ccm_monitor_agent_server.py`、`ccm_sub_agent_server.py`，通过 stdio 与 CLI 通信、通过 HTTP 调用 CCM API。Claude 配置文件动态生成到临时目录并在对应进程结束后清理

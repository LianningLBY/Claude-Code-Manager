...
- **Auto Capability policy（2026-08-06，执行入口仍关闭）**: `Task.capability_policy` 只允许本机普通 `mode=auto` Task 在创建时显式设置，SQL `NULL` 是唯一关闭态；V1 严格结构为 `{version:1,max_invocations,capabilities:{plan?,code_review?}}`，映射键同时是静态白名单和分类预算，总/分类都为正整数且硬上限 8。policy 按 Task incarnation 冻结，PUT、Manager→Worker create、migration、Shared shadow、Delivery/Plan/Loop/Goal 都 fail closed，clone 不继承。未来预算消费以 `source=agent_request` 的逻辑 Invocation 作不可退款 ledger（失败/取消/stale 也计数，Execution retry 和幂等重放不重复计），必须在同一 Task 锁事务内完成预算检查、Invocation/Execution 与 `waiting_capability` 状态提交。`AUTO_CAPABILITY_ENABLED=false` 是独立的新 admission 开关，并且还要求 Capability Core 开启；当前 `create_agent_invocation` 继续无条件拒绝，直到 exact output 解析与 durable resume outbox 一起接通。

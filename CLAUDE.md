...
- **MCP 架构**: `mcp_config.py` 先为主任务、Monitor、Sub-Agent 构造 provider-neutral `McpServerSpec`（command/args/cwd/env/required/enabled_tools/timeouts），再由 provider renderer 输出配置：Claude renderer 生成兼容的 `mcpServers` JSON；Codex renderer 生成 app-server 线程级 `config.mcp_servers` 对象，或从同一对象生成 `codex exec -c` TOML argv（纯内存、不写全局 `config.toml`）。当前生产注入仍仅走 Claude CLI，Codex renderer 尚未接线。三个 FastMCP server 分别是 `ccm_skills_server.py`、`ccm_monitor_agent_server.py`、`ccm_sub_agent_server.py`，通过 stdio 与 CLI 通信、通过 HTTP 调用 CCM API。Claude 配置文件动态生成到临时目录并在对应进程结束后清理

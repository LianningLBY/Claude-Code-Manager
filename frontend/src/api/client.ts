...
export type SSHProfileUpdateInput = Partial<SSHProfileInput> & {
  revision: number;
};
...
export const api = {
  ...
  updateSSHProfile: (id: number, data: SSHProfileUpdateInput) =>
    request<SSHProfile>(`/api/ssh-profiles/${id}`, {
      method: 'PUT',
      body: JSON.stringify(data),
    }),
  deleteSSHProfile: (id: number, revision: number) =>
    request<{ ok: boolean }>(`/api/ssh-profiles/${id}?revision=${revision}`, {
      method: 'DELETE',
    }),
